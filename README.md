...
After color events are received, each session will Publish its own measured latency in a message (type = report). When the `report` message events are received they will be displayed on screen so results from each active user session can be seen.
